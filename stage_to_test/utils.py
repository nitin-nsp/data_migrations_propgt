...
def get_table_data(table_name,src_db_name=src_db):
    ...
    with tar_conn.cursor() as cur:
        for data_row in data:
            try:
                ...
                with open(f'./failed_logs/load__{table_name}.json', 'a') as f:
                    f.write(json.dumps({"failed_row": data_row, "error": str(e), "traceback": traceback.format_exc()}) + '\n')
                    f.write("-----|10\n")
                    f.close()
